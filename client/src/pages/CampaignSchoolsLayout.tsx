...
import { ArrowLeft, List, PlusCircle, Search } from 'lucide-react';

export default function CampaignSchoolsLayout() {
  const { campaignId } = useParams<{ campaignId: string }>();
  const navigate = useNavigate();
  const location = useLocation();
  const base = `/campaigns/${campaignId}/schools`;

  const navItems = [
    { path: base, label: 'List', icon: List },
    { path: `${base}/new`, label: 'Add School', icon: PlusCircle },
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <header className="sticky top-0 z-20 bg-card/80 backdrop-blur-lg border-b border-border">
        <div className="max-w-3xl mx-auto px-4 h-14 flex items-center gap-3">
          ...
          <h1 className="text-lg font-semibold tracking-tight">Campaign Schools</h1>
        </div>
